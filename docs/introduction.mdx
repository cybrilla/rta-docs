...
POA APIs use [ONDC MF Specifications](https://ondc-official.github.io/ONDC-FIS-Specifications/?branch=release-FIS14-2.1.0). The APIs are asynchronous in nature, which means POA accepts the request (`action` call), sends an acknowledgement (`ACK`) and later respond back with the result of the request (`on_action` callback).
